...
begin
  ...
  if v_count <> 5 then
    raise exception 'phase5 verification failed: all 5 tables need RLS enabled and FORCE off (got %), because the SECURITY DEFINER RPCs write as the owner and a forced policy would block them';
  end if;
  ...
  select count(*) into v_count from pg_policies
   where schemaname = 'public' and tablename = 'notifications' and policyname = 'notifications: owner or broadcast read'
     and pg_get_expr(polqual, tableoid) like '%auth.uid()%';
  if v_count <> 1 then
  ...
  select count(*) into v_count from pg_indexes
   where schemaname = 'public'
     and indexname in ('notifications_user_dedupe_key','notification_jobs_dedupe_key',
                       -- a table constraint's index is named after the constraint, and these two are named
                       -- explicitly above; the verification has to spell them the way pg_indexes does.
