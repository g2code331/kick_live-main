...
| `../CREATE_ADMIN_PROFILE.sql`                                                                        | bootstrap-only, review before running                   | Grants the _first_ admin from a SQL editor as a superuser. Deliberately **not** in `SETUP.sql`: it names a real user and assumes a superuser session. Nothing in the app calls it; delete it once the instance has an admin.                                                                                                |
