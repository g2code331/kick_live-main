...
> Already set up and only deciding whether production can take a deploy? **[`docs/GO_LIVE.md`](GO_LIVE.md)** is the shorter list:
> one line per remaining action, with the proof each must print, and `npm run preflight:production` to check the repo half first.
...
| Queues, R2 buckets, Worker secrets, Pages projects, GitHub env vars    | **not done** — that is steps 2, 4 and 5; `npm run preflight:production` prints one line per gap    |
...
Then run `supabase/DB_CHECK.sql` in the **same editor**: one row per thing the app depends on, and every row must say
`pass`. It is read-only (no grant, no create, no drop) and it is the only fast way to tell "this project never got the
bundle" apart from "the bundle is in and something else is wrong" — which is the pair of states that produces a
confusing `401 … permission denied for table profiles` at sign-up. `npm run db:check` runs the same file locally against
a throwaway Postgres with the bundle applied, so a `fail` row you cannot explain is also a bug in the bundle.
...
In each project, after you have signed up once in the app (or created the user in Auth → Users), run
`CREATE_ADMIN_PROFILE.sql` — **one edit**: the `p_email := '…'` line becomes your address, or set `p_user_id`
instead and touch nothing else.

> **Type that address into the editor. Do not paste it.** A chat client, a ticket or a markdown preview turns
> `you@gmail.com` into `[you@gmail.com](mailto:you@gmail.com)` the moment it leaves the clipboard, and the paste
...
It first checks its own ground — that this is a Supabase project, that `public.profiles` exists, that the sign-up
function and `kicklive_set_user_role` are installed — and refuses with the fixing file named in the message. A project
that never got `SETUP.sql` therefore cannot be "admin'd" into a state where the app still does not work. (Its trailing
result query reads only the catalog for the same reason: an editor runs a paste statement by statement, and a second
unrelated error on top of the real one sends people to fix the wrong thing.)
...
(`public.kicklive_set_user_role()`, audited, last-admin-guarded); then delete the file from your working copy.
...
| --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| deep link 404s / a stale asset returns HTML                                                                     | the Pages contract files did not ship: `dist/web` must contain `_worker.js` (advanced mode) and `_headers` — `npm run verify` checks this pre-build                                                                                                                                                                                                                                                                               |
| `Account created, but the profile could not be saved` / `401 … permission denied for table profiles` on sign-up | the database is not the hardened one. Run `supabase/DB_CHECK.sql` in _that_ project: a `fail` on the trigger row means `SETUP.sql` was never pasted there (a new project has `profiles` from the base schema but no `on_auth_user_created`, so no row and no grants). Paste the bundle, sign out, register again — never a `grant`. If the row exists and only the write was refused, the account is fine and sign-up now says so |
| the profile page refuses to save, mentioning a permission error                                                 | the client no longer writes `public.profiles` at all (phases 1 and 10 removed that on purpose) — it calls `public.kicklive_profile_update()`. If that RPC is missing, this project has a half-applied bundle: re-paste `SETUP.sql`                                                                                                                                                                                                |
