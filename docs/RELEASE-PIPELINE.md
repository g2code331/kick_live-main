...
|                    |                                                                       | `windows`                                                                                                  | `windows-latest`: `build` + `package:win` (NSIS installer), hashed into `checksums-win.json`; feeds the manifest's `win32_x64` entry (kind `nsis`)                                                             | `desktop-win32-x64`                                                                     |
|                    |                                                                       | `release`                                                                                                  | **manifest built from the real artifact bytes** (linux_x64 + win32_x64), validated, GitHub Release, published-manifest + sha256 re-verification for both                                                       | `release-manifests`                                                                     |
| `build-mobile.yml` | PR, push `main`, `v*` tag, dispatch                                   | `android` / `ios`                                                                                          | `ubuntu`: JDK 21 + Android SDK, `mobile:sync:android`, `gradlew assemble{Debug,Release}` → APK. `macos`: `mobile:sync:ios`, `pod install`, unsigned `xcodebuild`. Tag builds attach to the release (6× retry). | `android-apk`, `ios-app`                                                                |
...
| `full-stack.yml`   | push to `main`; manual, with `apply_cloudflare` / `deploy_production` | pair-check → gates + executed SQL → Cloudflare provisioning → staging migrations → staging Worker → verify | nothing (a push never reaches production)                                                                                                                                                                      |
...
| ------------------------ | ------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `VITE_SUPABASE_URL`      | yes (build jobs)    | web, desktop, release, deploy  | **no error from GitHub** — `${{ secrets.X }}` renders empty, Vite inlines the fallback URL baked in `src/lib/supabase.ts`, and the app ships pointed at the wrong project. `check-secrets.mjs` fails the job instead: `::error::secret VITE_SUPABASE_URL is unset`. Since the mode files are tracked, a _fresh clone_ also passes that audit with the value marked `set (from .env.production)` — the check reports where a value came from, so "configured" and "configured from the committed default" are distinguishable. |
...
npm run verify             # 20 checks: branding, version, 3× typecheck, hooks, yaml, Pages contract, lockfile,
                           #                        mode-file sync, and the migrations executed on PGlite
...
- **Windows (`win32_x64`) is now built and self-updating.** The `windows` job in `release.yml` builds an
  NSIS installer on a `windows-latest` runner and the `release` job folds it into the manifest, so desktop
  auto-update covers Windows the same way it covers Linux. The installer is **unsigned** — SmartScreen warns
  on first run; wiring a code-signing cert is a `win.certificateFile`/`certificatePassword` pair (or Azure
  Trusted Signing) in `electron-builder.yml` plus the matching secrets, nothing else changes.
- **Android/iOS ship from `build-mobile.yml`, not the update manifest.** Mobile self-update is the app stores,
  so the APK/`.app` are attached to the GitHub Release for download/store upload and are intentionally absent
  from `platforms.*`. The APK is debug-key-signed unless `ANDROID_KEYSTORE_BASE64` (+ password/alias) secrets
  are set; iOS is built with signing disabled (an unsigned `.app` for simulator/inspection). A store-ready
  signed IPA needs an Apple developer cert and is the remaining follow-up.
- `darwin_arm64` (macOS desktop) still exists in the schema for the _contract_ only; no job builds it.
