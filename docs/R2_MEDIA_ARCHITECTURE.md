...
**Unit** — `tests/unit/phase6-media.test.ts`: the sniffing table (each accepted format, the spoof case, markup, truncation, dimension ceilings), key derivation and the traversal regexes, the cache-class rules, the policy↔SQL agreement (kind CHECK, URL columns, quotas, the byte ceiling against `MEDIA_MAX_BYTES`), every existing column's preservation, the migration's destructive-statement and RLS-posture text tests with the function census derived from its own `$verify$` list, the route table's media entries, the retired-stub absence, the README map, the no-`supabase.storage`-in-`src/**` census, the credential census, `assetUrl` pass-through behaviour, and the secret scanner against a fixture tree it builds itself.
