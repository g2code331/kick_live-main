...
async function render(env, vars) {
  ...
  // VITE_API_BASE_URL is empty by default: the app talks to same-origin `/api`, which the custom domain's
  // zone route serves. A raw `<project>.pages.dev` host has no such route, so an optional `WEB_API_ORIGIN`
  // in `[env.<env>.vars]` may name the deployed Worker's absolute origin to call directly. When set it is
  // baked here (and the browser then makes a cross-origin call, so that pages.dev origin must appear in the
  // Worker's ALLOWED_ORIGINS). When unset the base stays empty and same-origin `/api` is used, unchanged.
  const apiOrigin = String(vars.WEB_API_ORIGIN || "").trim();
  if (apiOrigin && !/^https:\/\/[a-z0-9.-]+$/i.test(apiOrigin)) {
    throw new Error(`${CONFIG}: [env.${env}.vars] WEB_API_ORIGIN "${apiOrigin}" must be a bare https origin like https://kicklive-api-staging.<subdomain>.workers.dev`);
  }
  return (
