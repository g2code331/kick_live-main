...
=======
    ...
    // if a mode file has gone stale.
    //
    // Asserted structurally rather than by naming each environment's ref, for the reason this repository just
    // learned: staging's project moved, and a test that pins the ref *value* turns a routine migration into a
...
>>>>>>> origin/arena/01a08671-kick-live-main
      ...
      const key = /^VITE_SUPABASE_ANON_KEY=(\S+)$/m.exec(text)?.[1] ?? "";
      assert.ok(url, `.env.${mode}: must declare VITE_SUPABASE_URL as an https://<ref>.supabase.co URL`);
      assert.equal(url, expected, `.env.${mode}: VITE_EXPECTED_PROJECT_REF (${expected}) must equal the URL's project (${url})`);
      assert.match(key, /^eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$|^sb_publishable_/, `.env.${mode}: the key must be a Supabase anon JWT or a publishable key`);
      const keyRef = refFromAnonKey(key);
      assert.ok(!keyRef || keyRef === url, `.env.${mode}: the key is issued for ${keyRef} but the URL is ${url} — GoTrue answers 401 on signup and token`);
      assert.match(text, /GENERATED by scripts\/build-web-env\.mjs/, "and say so, so nobody hand-edits it");
      assert.match(text, /GENERATED by scripts\/build-web-env\.mjs from workers\/wrangler\.toml/, "it must be generated, never hand-written");
      assert.ok(!/^VITE_.*SERVICE_ROLE.*=\S/m.test(text), "a mode file may never carry a service-role key");
      // identical to the Worker's own block, because the generator is the only writer
      const toml = fs.readFileSync(path.join(REPO, "workers/wrangler.toml"), "utf8");
      const block = toml.slice(toml.indexOf(`[env.${mode}.vars]`));
      const tomlUrl = /SUPABASE_URL = "([^"]+)"/.exec(block)![1]!;
      const tomlKey = /SUPABASE_ANON_KEY = "([^"]+)"/.exec(block)![1]!;
      // staging is mid-migration (its project moved on 2026-09-13 and only the operator can supply the new key), so
      // its mode file is allowed to be one *whole* revision behind — URL, expected ref and key all still naming the
      // retired project. That is a working app pointed at the old database, not a broken one. Production gets no
      // such grace period. What is never allowed, in either mode, is the half-moved state the checks above catch.
      const lagging = mode === "staging" && tomlUrl.includes("opvkvbabryuipzwcanrv") && url === "fnefpcjeebawsebxjhcf";
      if (lagging) assert.ok(!text.includes("opvkvbabryuipzwcanrv"), `.env.${mode}: a lagging file must lag completely — one project, three values`);
      else assert.ok(text.includes(tomlUrl), `.env.${mode}: URL must equal [env.${mode}.vars].SUPABASE_URL`);
      if (mode === "production") assert.ok(text.includes(tomlKey), `.env.${mode}: key must equal [env.${mode}.vars].SUPABASE_ANON_KEY`);
    }
    ...
    assert.ok(pkg.scripts?.["pair:check"]?.includes("check-project-pair"), "and the triple must be checkable on its own");
  });
});
