...
3. Pages projects (wrangler can create them, but the dashboard is equivalent):
   ```bash
   ...
   ```
4. Custom domains — Pages → project → Settings → Functions/(domains) → *Set up a custom domain*:
...
5. GitHub: Settings → Environments → create `production` and `staging` (and `preview` if you want
